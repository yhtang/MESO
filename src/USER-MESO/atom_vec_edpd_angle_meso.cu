...
AtomVecEDPDAngle::AtomVecEDPDAngle( LAMMPS *lmp ) :
    AtomVecDPDAngle( lmp ),
    dev_Q( lmp, "AtomVecEDPDAngle::dev_q" ),
    dev_T( lmp, "AtomVecEDPDAngle::dev_T" ),
    dev_T_pinned( lmp, "AtomVecEDPDAngle::dev_T_pinned" ),
    dev_Q_pinned( lmp, "AtomVecEDPDAngle::dev_Q_pinned" ),
	dev_therm_merged( lmp, "AtomVecEDPDAngle::dev_therm_merged" )
{
    comm_x_only    = 0;
    comm_f_only    = 0;
    mass_type      = 1;
    ...
    pre_output   = AtomAttribute::LOCAL  | AtomAttribute::ESSENTIAL | AtomAttribute::TEMP | AtomAttribute::FORCE |
                   AtomAttribute::MOLE   | AtomAttribute::EXCL      | AtomAttribute::BOND | AtomAttribute::ANGLE;

    Q = T = NULL;
}

void AtomVecEDPDAngle::copy( int i, int j, int delflag )
{
	tag[j] = tag[i];
    type[j] = type[i];
    mask[j] = mask[i];
...
void AtomVecEDPDAngle::grow( int n )
{
    unpin_host_array();
...
void AtomVecEDPDAngle::grow_cpu( int n )
{
    AtomVecDPDAngle::grow_cpu( n );

    T = memory->grow( atom->T, nmax, "atom:T" );
    Q = memory->grow( atom->Q, nmax, "atom:Q" );
}

void AtomVecEDPDAngle::grow_device( int nmax_new )
{
    AtomVecDPDAngle::grow_device( nmax_new );
...
void AtomVecEDPDAngle::pin_host_array()
{
    AtomVecDPDAngle::pin_host_array();

    if( atom->T ) dev_T_pinned.map_host( atom->nmax, atom->T );
    if( atom->Q ) dev_Q_pinned.map_host( atom->nmax, atom->Q );
}

void AtomVecEDPDAngle::unpin_host_array()
{
    AtomVecDPDAngle::unpin_host_array();

    dev_T_pinned.unmap_host( atom->T );
    dev_Q_pinned.unmap_host( atom->Q );
}

void AtomVecEDPDAngle::transfer_impl(
...
{
    AtomVecDPDAngle::transfer_impl( events, per_atom_prop, direction, p_beg, n_atom, p_stream, p_inc, permute_to, permute_from, action, streamed );
...
__global__ void gpu_merge_xvtT__(
...
void AtomVecEDPDAngle::dp2sp_merged( int seed, int p_beg, int p_end, bool offset )
{
    r64 cx = 0., cy = 0., cz = 0.;
    if( offset ) {
    ...
    if( !grid_cfg.x ) {
        grid_cfg = meso_device->occu_calc.right_peak( 0, gpu_merge_xvtT__, 0, cudaFuncCachePreferL1 );
        cudaFuncSetCacheConfig( gpu_merge_xvtT__, cudaFuncCachePreferL1 );
    }

    gpu_merge_xvtT__ <<< grid_cfg.x, grid_cfg.y, 0, meso_device->stream() >>> (
...
int AtomVecEDPDAngle::pack_comm( int n, int *list, double *buf,
...
int AtomVecEDPDAngle::pack_comm_vel( int n, int *list, double *buf,
                                    int pbc_flag, int *pbc )
{
    int i, j, m;
    double dx, dy, dz;
...
void AtomVecEDPDAngle::unpack_comm( int n, int first, double *buf )
{
    int i, m, last;
...
void AtomVecEDPDAngle::unpack_comm_vel( int n, int first, double *buf )
{
    int i, m, last;
...
int AtomVecEDPDAngle::pack_border( int n, int *list, double *buf,
                                  int pbc_flag, int *pbc )
{
    int i, j, m;
    double dx, dy, dz;
...
int AtomVecEDPDAngle::pack_border_vel( int n, int *list, double *buf,
                                      int pbc_flag, int *pbc )
{
    int i, j, m;
    double dx, dy, dz;
...
void AtomVecEDPDAngle::unpack_border( int n, int first, double *buf )
{
    int i, m, last;
...
void AtomVecEDPDAngle::unpack_border_vel( int n, int first, double *buf )
{
    int i, m, last;
...
int AtomVecEDPDAngle::pack_exchange( int i, double *buf )
{
    int m = 1;
    buf[m++] = x[i][0];
    buf[m++] = x[i][1];
...
int AtomVecEDPDAngle::unpack_exchange( double *buf )
{
    ...
    for( int k = 0; k < num_angle[nlocal]; k++) {
...
int AtomVecEDPDAngle::size_restart()
{
    int n = 0;
    for( int i = 0; i < atom->nlocal; i++ ) n += 15 + 2 * num_bond[i] + 4 * num_angle[i];
...
int AtomVecEDPDAngle::pack_restart( int i, double *buf )
{
    int m = 1;
    buf[m++] = x[i][0];
    buf[m++] = x[i][1];
...
int AtomVecEDPDAngle::unpack_restart( double *buf )
{
    int nlocal = atom->nlocal;
    if( nlocal == nmax ) {
        grow( 0 );
...
void AtomVecEDPDAngle::data_atom( double *coord, int imagetmp, char **values )
{
    int nlocal = atom->nlocal;
    if( nlocal == nmax ) grow( 0 );
...
//int AtomVecEDPDAngle::data_atom_hybrid(int nlocal, char **values)
//{
//  v[nlocal][0] = 0.0;
//  v[nlocal][1] = 0.0;
//  v[nlocal][2] = 0.0;
...
bigint AtomVecEDPDAngle::memory_usage()
{
    bigint bytes = 0;
...
void AtomVecEDPDAngle::force_clear( AtomAttribute::Descriptor range, int vflag )
{
