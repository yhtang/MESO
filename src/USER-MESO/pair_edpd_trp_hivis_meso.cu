...
MesoPairEDPDTRPHiVis::MesoPairEDPDTRPHiVis( LAMMPS *lmp ) : MesoPairEDPDTRPBase( lmp ), MesoPointers( lmp ) {
	s = NULL;
	split_flag  = 1;
    random = NULL;
}

MesoPairEDPDTRPHiVis::~MesoPairEDPDTRPHiVis()
{
    if( allocated ) {
...
void MesoPairEDPDTRPHiVis::allocate()
{
    allocated = 1;
    int n = atom->ntypes;
...
void MesoPairEDPDTRPHiVis::prepare_coeff()
{
    ...
    for( int i = 1; i <= n; i++ ) {
        for( int j = 1; j <= n; j++ ) {
            ...
            coeff_table[ cid * n_coeff + p_s     ] = s[i][j];
...
template<int evflag>
__global__ void gpu_pnipam_hivis(
        ...
        if( i >= p_beg ) {
            ...
            for( int p = 0; p < n_pair; p++ ) {
                ...
                if( rsq < power<2>( coeff_ij[p_cut] ) && rsq >= EPSILON_SQ ) {
                    ...
                    r64 wr       = 1.0 - __powd( r * coeff_ij[p_cutinv], coeff_ij[p_s] );
                    r64 wd       = wr * wr;
...
void MesoPairEDPDTRPHiVis::compute_kernel( int eflag, int vflag, int p_beg, int p_end )
{
    prepare_coeff();
    ...
    if( eflag || vflag ) {
        ...
        static GridConfig grid_cfg = meso_device->configure_kernel( gpu_pnipam_hivis<1>, shared_mem_size );
        gpu_pnipam_hivis<1> <<< grid_cfg.x, grid_cfg.y, shared_mem_size, meso_device->stream() >>> (
        ...
        // evaluate force only
        static GridConfig grid_cfg = meso_device->configure_kernel( gpu_pnipam_hivis<0>, shared_mem_size );
        gpu_pnipam_hivis<0> <<< grid_cfg.x, grid_cfg.y, shared_mem_size, meso_device->stream() >>> (
            meso_atom->tex_coord_merged, meso_atom->tex_veloc_merged, meso_atom->tex_misc("therm"),
            meso_atom->dev_force (0), meso_atom->dev_force (1), meso_atom->dev_force (2),
            meso_atom->dev_Q,
            meso_atom->dev_virial(0), meso_atom->dev_virial(1), meso_atom->dev_virial(2),
...
void MesoPairEDPDTRPHiVis::compute_bulk( int eflag, int vflag )
{
...
void MesoPairEDPDTRPHiVis::compute_border( int eflag, int vflag )
{
    int p_beg, p_end, c_beg, c_end;
...
void MesoPairEDPDTRPHiVis::compute( int eflag, int vflag )
{
    int p_beg, p_end, c_beg, c_end;
...
uint MesoPairEDPDTRPHiVis::seed_now() {
	return premix_TEA<64>( seed, update->ntimestep );
}

void MesoPairEDPDTRPHiVis::settings( int narg, char **arg )
{
...
void MesoPairEDPDTRPHiVis::coeff( int narg, char **arg )
{
    if( narg < 8 )
    ...
    double s_one     = atof( arg[4] );
    double cut_one   = atof( arg[5] );
    double cv_one    = atof( arg[6] );
    double kappa_one = atof( arg[7] );
    double theta_one = 1.0;
    double da_one    = 0.0;
    double omega_one = 1.0;
    if ( narg > 8 ) {
    	theta_one = atof( arg[8] );
    	da_one    = atof( arg[9] );
    	omega_one = atof( arg[10] );
    }
    ...
    for( int i = ilo; i <= ihi; i++ ) {
        for( int j = MAX( jlo, i ); j <= jhi; j++ ) {
            ...
            if (comm->me == 0)
            	printf("%d %d: a0 %lf gamma %lf s %lf cut %lf cv %lf kappa %lf theta %lf da %lf omega %lf\n", i, j, a0_one, gamma_one, s_one, cut_one, cv_one, kappa_one, theta_one,da_one, omega_one );
...
void MesoPairEDPDTRPHiVis::init_style()
{
...
double MesoPairEDPDTRPHiVis::init_one( int i, int j )
{
    if( setflag[i][j] == 0 )
...
void MesoPairEDPDTRPHiVis::write_restart( FILE *fp )
{
...
void MesoPairEDPDTRPHiVis::read_restart( FILE *fp )
{
        ...
        for( j = i; j <= atom->ntypes; j++ ) {
            ...
            if( setflag[i][j] ) {
                if( me == 0 ) {
                    ...
                    fread( &s[i][j], sizeof( double ), 1, fp );
                ...
                MPI_Bcast( &s[i][j], 1, MPI_DOUBLE, 0, world );
...
void MesoPairEDPDTRPHiVis::write_restart_settings( FILE *fp )
{
...
void MesoPairEDPDTRPHiVis::read_restart_settings( FILE *fp )
{
    if( comm->me == 0 ) {
...
double MesoPairEDPDTRPHiVis::single( int i, int j, int itype, int jtype, double rsq,
                               double factor_coul, double factor_dpd, double &fforce )
{
    error->warning( FLERR, "<MESO> MesoPairEDPDTRPHiVis::single not implemented" );
    return 0.;
}
