...
MesoBondWLCeDPD::MesoBondWLCeDPD( LAMMPS *lmp ):
    Bond(lmp),
    MesoPointers( lmp ),
    dev_lda( lmp, "MesoBondWLCeDPD::dev_lda" ),
    dev_Lsp( lmp, "MesoBondWLCeDPD::dev_Lsp" )
{
    kB = 1.0;
    coeff_alloced = 0;
}

MesoBondWLCeDPD::~MesoBondWLCeDPD()
{
	if (setflag) memory->destroy(setflag);
}

void MesoBondWLCeDPD::allocate_gpu()
{
...
void MesoBondWLCeDPD::allocate_cpu()
{
	allocated = 1;
	int n = atom->nbondtypes;
	lda.resize(n+1);
...
template<int evflag>
__global__ void gpu_bond_wlc_edpd(
    texobj tex_coord,
    texobj tex_therm,
    ...
    const int n_local )
{
    ...
    for( int i = blockIdx.x * blockDim.x + threadIdx.x; i < n_local ; i += gridDim.x * blockDim.x ) {
        ...
        for( int p = 0 ; p < n ; p++ ) {
            ...
            r64 r     = rinv * rsq ;
            r64 R     = r / Lsp[type];
            r64 T     = ( therm1.T + therm2.T ) * 0.5;
            r64 fbond = kB * T / lda[type] * ( 0.25 / power<2>( 1 - R ) - 0.25 + R );

            fx += dx * fbond ;
            fy += dy * fbond ;
            fz += dz * fbond ;
            if( evflag ) e += kB * T / ( 4. * lda[type] ) * ( Lsp[type]*Lsp[type]/( Lsp[type] - r ) - r + (2.*r*r) / Lsp[type] );
        }
...
void MesoBondWLCeDPD::compute( int eflag, int vflag )
{
    if( !coeff_alloced ) allocate_gpu();

    static GridConfig grid_cfg, grid_cfg_EV;
    if( !grid_cfg_EV.x ) {
        grid_cfg_EV = meso_device->occu_calc.right_peak( 0, gpu_bond_wlc_edpd<1>, ( atom->nbondtypes + 1 ) * 2 * sizeof( r64 ), cudaFuncCachePreferL1 );
        cudaFuncSetCacheConfig( gpu_bond_wlc_edpd<1>, cudaFuncCachePreferL1 );
        grid_cfg    = meso_device->occu_calc.right_peak( 0, gpu_bond_wlc_edpd<0>, ( atom->nbondtypes + 1 ) * 2 * sizeof( r64 ), cudaFuncCachePreferL1 );
        cudaFuncSetCacheConfig( gpu_bond_wlc_edpd<0>, cudaFuncCachePreferL1 );
    }
    ...
    if( eflag || vflag ) {
        gpu_bond_wlc_edpd<1> <<< grid_cfg_EV.x, grid_cfg_EV.y, ( atom->nbondtypes + 1 ) * 2 * sizeof( r64 ), meso_device->stream() >>> (
        ...
        gpu_bond_wlc_edpd<0> <<< grid_cfg.x, grid_cfg.y, ( atom->nbondtypes + 1 ) * 2 * sizeof( r64 ), meso_device->stream() >>> (
            meso_atom->tex_coord_merged,
            meso_atom->tex_misc("therm"),
            meso_atom->dev_force[0],
            meso_atom->dev_force[1],
...
void MesoBondWLCeDPD::settings(int narg, char **arg)
{
	for(int i = 0 ; i < narg ; i++) {
		if ( !strcmp( arg[i], "kB" ) ) {
			kB = force->numeric(FLERR,arg[++i]);
		} else {
			error->warning(FLERR,"unknown parameter for bond style wlc/edpd/meso");
		}
	}
}
...
void MesoBondWLCeDPD::coeff(int narg, char **arg)
{
...
double MesoBondWLCeDPD::equilibrium_distance(int i)
{
...
void MesoBondWLCeDPD::write_restart(FILE *fp)
{
...
void MesoBondWLCeDPD::read_restart(FILE *fp)
{
  allocate_cpu();
...
void MesoBondWLCeDPD::write_data(FILE *fp)
{
...
double MesoBondWLCeDPD::single(int type, double rsq, int i, int j,
                        double &fforce)
{
  error->warning(FLERR,"<MESO> MesoBondWLCeDPD::single not implemented");
	return 0.;
}
