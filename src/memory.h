...
------------------------------------------------------------------------- */
  ...
  template <typename TYPE>
    TYPE **create(TYPE **&array, int n, const char *name) {return fail<TYPE**>(name);}
...
------------------------------------------------------------------------- */
  ...
  template <typename TYPE>
    TYPE **grow(TYPE **&array, int n, const char *name) {return fail<TYPE**>(name);}
...
------------------------------------------------------------------------- */
  ...
  template <typename TYPE>
    TYPE **grow_soa(TYPE **&array, int n1, int n2, int n2old, const char *name)
    {
      if (array == NULL) return create(array,n1,n2,name);

      TYPE * new_data  = (TYPE * ) smalloc(((bigint) sizeof(TYPE  )) * n1*n2, name);
      TYPE **new_array = (TYPE **) smalloc(((bigint) sizeof(TYPE *)) * n1,    name);

      for(int i = 0; i < n1; i++) {
        new_array[i] = &new_data[i * n2];
        memcpy( new_array[i], array[i], sizeof(TYPE) * n2old );
      }
...
------------------------------------------------------------------------- */
  ...
  template <typename TYPE>
    TYPE ***create2d_offset(TYPE ***&array, int n1, int n2lo, int n2hi,
                            const char *name) {return fail<TYPE***>(name);}
