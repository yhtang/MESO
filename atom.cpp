...
void Atom::data_bonds(int n, char *buf)
{
  ...
  for (int i = 0; i < n; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    int count = sscanf(buf,"%d %d %d %d %lf",&tmp,&itype,&atom1,&atom2,&r0);
    ...
    if ((m = map(atom1)) >= 0) {
      ...
      if ( count > 4 && bond_r0 ) bond_r0[m][num_bond[m]] = r0;
      num_bond[m]++;
    }
    if (newton_bond == 0) {
      if ((m = map(atom2)) >= 0) {
...
void Atom::data_angles(int n, char *buf)
{
  ...
  for (int i = 0; i < n; i++) {
    next = strchr(buf,'\n');
    *next = '\0';
    int count = sscanf(buf,"%d %d %d %d %d %lf",&tmp,&itype,&atom1,&atom2,&atom3,&a0);
    if (atom1 <= 0 || atom1 > map_tag_max ||
        atom2 <= 0 || atom2 > map_tag_max ||
    ...
    if ((m = map(atom2)) >= 0) {
      ...
      if ( count > 5 && angle_a0 ) angle_a0[m][num_angle[m]] = a0;
      num_angle[m]++;
    }
    if (newton_bond == 0) {
      if ((m = map(atom1)) >= 0) {
