...
MesoPairEDPDTRPFast::MesoPairEDPDTRPFast( LAMMPS *lmp ) : MesoPairEDPDTRPBase( lmp ), MesoPointers( lmp )
{
    split_flag  = 1;
    random = NULL;
}

MesoPairEDPDTRPFast::~MesoPairEDPDTRPFast()
{
    if( allocated ) {
...
void MesoPairEDPDTRPFast::allocate()
{
    allocated = 1;
    int n = atom->ntypes;
...
void MesoPairEDPDTRPFast::prepare_coeff()
{
...
template<int evflag>
__global__ void gpu_pnipam_fast(
    ...
    for( int iter = blockIdx.x * blockDim.x + threadIdx.x; ; iter += gridDim.x * blockDim.x ) {
        ...
        if( i >= p_beg ) {
            ...
            for( int p = 0; p < n_pair; p++ ) {
                ...
                if( rsq < coeff_ij[p_cut] * coeff_ij[p_cut] && rsq >= 1e-9 ) {
                    ...
                    r32 dot_rinv = dot * rinv;
                    r32 wc       = 1.f - r * coeff_ij[p_cutinv];
                    r32 wr       = wc;
                    r32 wd       = wr * wr;

                    tmm therm2   = tex1Dfetch<float4>( tex_therm, j );
                    r32 T2_inv   = 1.f / therm2.T;
                    r32 T_ij     = 0.5f * ( therm1.T + therm2.T );
                    r32 gamma_ij = coeff_ij[p_gamma];
                    r32 sigma_ij = 2.0f * sqrtf( gamma_ij / ( T1_inv + T2_inv ) );

                    // force
                    {
                        r32 rn       = gaussian_TEA_fast<4>( veloc1.i > veloc2.i, veloc1.i, veloc2.i );
                        r32 alpha_ij = coeff_ij[p_a0] * T_ij; // DPD classical term
                        r32 d_alpha  = coeff_ij[p_da] / ( 1.f + expf( coeff_ij[p_omega] * ( T_ij - coeff_ij[p_theta]  ) ) ); // temperature-dependence

                        r32 fpair  =  ( alpha_ij + d_alpha ) * wc
                        ...
                        fz += dz * fpair;
                        if( evflag ) {
                            energy += 0.5f * alpha_ij * coeff_ij[p_cut] * wc * wc;
                            vrxx += dx * dx * fpair;
                            vryy += dy * dy * fpair;
                            vrzz += dz * dz * fpair;
                            vrxy += dx * dy * fpair;
                    ...
                    // heat
                    {
                        r32 wr_t  = wc;
                        r32 wc_t  = wr_t * wr_t;
                        r32 cv    = coeff_ij[p_cv];
                        r32 beta  = coeff_ij[p_sq2kpa] * cv * T_ij;
                        r32 kij   = beta * beta * 0.5f;

                        r32 rn2      = gaussian_TEA_fast<4>( therm1.i < therm2.i, therm1.i, therm2.i );
                        r32 mass_inv = sqrtf( therm1.mass_inv * therm2.mass_inv );

                        r32 qpair    = kij * wc_t * ( T1_inv - T2_inv )
                                       + 0.5f / cv * ( wd * ( gamma_ij * dot_rinv * dot_rinv - sigma_ij * sigma_ij * mass_inv ) - sigma_ij * wr * dot_rinv * rn2 )
...
void MesoPairEDPDTRPFast::compute_kernel( int eflag, int vflag, int p_beg, int p_end )
{
    prepare_coeff();
    ...
    if( eflag || vflag ) {
        ...
        static GridConfig grid_cfg = meso_device->configure_kernel( gpu_pnipam_fast<1>, shared_mem_size );
        gpu_pnipam_fast<1> <<< grid_cfg.x, grid_cfg.y, shared_mem_size, meso_device->stream() >>> (
        ...
        // evaluate force only
        static GridConfig grid_cfg = meso_device->configure_kernel( gpu_pnipam_fast<0>, shared_mem_size );
        gpu_pnipam_fast<0> <<< grid_cfg.x, grid_cfg.y, shared_mem_size, meso_device->stream() >>> (
            meso_atom->tex_coord_merged, meso_atom->tex_veloc_merged, meso_atom->tex_misc("therm"),
            meso_atom->dev_force [0], meso_atom->dev_force [1], meso_atom->dev_force [2],
            meso_atom->dev_Q,
            meso_atom->dev_virial[0], meso_atom->dev_virial[1], meso_atom->dev_virial[2],
...
void MesoPairEDPDTRPFast::compute_bulk( int eflag, int vflag )
{
...
void MesoPairEDPDTRPFast::compute_border( int eflag, int vflag )
{
    int p_beg, p_end, c_beg, c_end;
...
void MesoPairEDPDTRPFast::compute( int eflag, int vflag )
{
    int p_beg, p_end, c_beg, c_end;
...
uint MesoPairEDPDTRPFast::seed_now() {
	return premix_TEA<64>( seed, update->ntimestep );
}

void MesoPairEDPDTRPFast::settings( int narg, char **arg )
{
...
void MesoPairEDPDTRPFast::coeff( int narg, char **arg )
{
    if( narg < 7 )
...
void MesoPairEDPDTRPFast::init_style()
{
...
double MesoPairEDPDTRPFast::init_one( int i, int j )
{
    if( setflag[i][j] == 0 )
...
void MesoPairEDPDTRPFast::write_restart( FILE *fp )
{
...
void MesoPairEDPDTRPFast::read_restart( FILE *fp )
{
...
void MesoPairEDPDTRPFast::write_restart_settings( FILE *fp )
{
...
void MesoPairEDPDTRPFast::read_restart_settings( FILE *fp )
{
    if( comm->me == 0 ) {
...
double MesoPairEDPDTRPFast::single( int i, int j, int itype, int jtype, double rsq,
                               double factor_coul, double factor_dpd, double &fforce )
{
    error->warning( FLERR, "<MESO> MesoPairEDPDTRPFast::single not implemented" );
    return 0.;
}
